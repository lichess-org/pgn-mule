      ...
      if (minutes >= minutesInactivityDie) {
        ...
        if (minutes >= minutesInactivitySlowDown) {
          ...
          if (Math.abs(secondsSinceUpdated - source.delay / 1000.0) < Math.abs(secondsSinceUpdated - slowPollRate)) {
            say(`${name} Slowing refresh to ${delay / 1000} seconds`, defaultChannel);
          }
        }
...
let react = async (name: string, channel: any, ts: any) => {
  ...
  console.log(`Successfully added reaction ${result.ts} in conversation ${channel}`);
};
...
let addOrSet = async (parts: string[], event: any, shouldReact: boolean = true) => {
  let delay = 10;
  if (parts.length > 3) {
...
(async () => {
  ...
  router.get('/:names+', async (ctx, _) => {
    ...
    let sources = dbg(await Promise.all(names.map(n => getSource(n as string))));
    await Promise.all(
